...
export function renderRichText(content: any): string {
  ...
  // Debug: log the content structure (only in development)
  if (typeof window === "undefined" && import.meta.env.DEV) {
    // Server-side only, development only
...
function renderBlock(block: RichTextNode): string {
  ...
  switch (block.type) {
    ...
    case "preformatted":
      ...
      const hasHtmlTags =
        preText.includes("<table") ||
        preText.includes("<div") ||
        preText.includes("<p") ||
        preText.includes("<h") ||
        preText.includes("<thead") ||
        preText.includes("<tbody") ||
        preText.includes("<tr") ||
        preText.includes("<td") ||
        preText.includes("<th") ||
        preText.includes("&lt;table") ||
    ...
    case "image":
      ...
      // ✅ SECURITY: Validate image URL to prevent SSRF and XSS
      if (!isValidImageUrl(imgUrl)) {
        console.warn("Invalid image URL detected, skipping:", imgUrl);
        return "";
      }
    ...
    case "embed":
      const embedUrl =
        (block as any).oembed?.embed_url || (block as any).url || "";
      const embedHtml = (block as any).oembed?.html || "";
      if (embedHtml) {
        // ✅ SECURITY: Sanitize embed HTML to prevent XSS
        const sanitizedEmbed = sanitizeHtml(embedHtml);
        return `<div class="embed">${sanitizedEmbed}</div>`;
      }
      // ✅ SECURITY: Validate embed URL before rendering iframe
      ...
      if (embedUrl) {
        console.warn("Invalid embed URL detected, skipping:", embedUrl);
      }
      return "";
  ...
  for (const span of sortedSpans) {
    ...
    switch (span.type) {
      ...
      case "hyperlink":
        ...
        text =
          before +
          `<a href="${escapeHtml(url)}" target="${target}" rel="${
